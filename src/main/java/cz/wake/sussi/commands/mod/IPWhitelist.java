...
public class IPWhitelist implements ICommand {
    ...
    public void onCommand(User sender, MessageChannel channel, Message message, String[] args, Member member, EventWaiter w) {
        if(!sender.getId().equals("238410025813540865") || !sender.getId().equals("177516608778928129")){
            channel.sendMessage(MessageUtils.getEmbed(Color.RED).setTitle("Chyba při vykonáváni příkazu").setDescription("Na toto mají právo pouze Krosta a Kwak!").build()).queue();
            return;
        }
        ...
        if(args[0].equals("add")){
            if (args.length < 3) {
                channel.sendMessage(MessageUtils.getEmbed(Color.RED).setTitle("Chyba při vykonáváni příkazu").setDescription("Nesprávně napsané argumenty! Př. ,ipwl add 1.1.1.1 Testovaci Zprava").build()).queue();
                return;
            }
        ...
        if(args[0].equals("remove")){
            if (args.length < 2) {
                channel.sendMessage(MessageUtils.getEmbed(Color.RED).setTitle("Chyba při vykonáváni příkazu").setDescription("Nesprávně napsané argumenty! Př. ,ipwl remove 1.1.1.1").build()).queue();
                return;
            }
