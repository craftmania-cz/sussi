...
public class NewAts implements ICommand {
    ...
    public void onCommand(User sender, MessageChannel channel, Message message, String[] args, Member member, EventWaiter w) {
        if (args.length < 1) {
            ...
            if (sender.getIdLong() == Sussi.getConfig().getOwnerID()) {
                if (args.length > 1) {
                    String name = args[1];
    ...
    private void secondPage(User s, Message message, MessageChannel ch, EventWaiter w, ATS ats) {
        message.editMessage(MessageUtils.getEmbed(ats.getColorByRank())
                ...
                .addField("Hardcore Vanilla", "**Chat:** " + ats.getServerATS(ATS.Server.HARDCORE_VANILLA).getChatBody() + "\n**Odehráno:** " + TimeUtils.formatTime("%d dni, %hh %mm", ats.getServerATS(ATS.Server.HARDCORE_VANILLA).getPlayedTime(), false) + "\n**Poslední aktivita:** " + getDate(ats.getServerATS(ATS.Server.HARDCORE_VANILLA).getLastActivity()), true)
