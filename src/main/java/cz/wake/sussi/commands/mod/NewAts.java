...
public class NewAts implements ICommand {
    ...
    private void secondPage(User s, Message message, MessageChannel ch, EventWaiter w, ATS ats) {
        message.editMessage(MessageUtils.getEmbed(ats.getColorByRank())
                ...
                .addField("Vanilla: Anarchy", "**Chat:** " + ats.getServerATS(ATS.Server.VANILLA_ANARCHY).getChatBody() + "\n**Odehráno:** " + TimeUtils.formatTime("%d dni, %hh %mm", ats.getServerATS(ATS.Server.VANILLA_ANARCHY).getPlayedTime(), false) + "\n**Poslední aktivita:** " + getDate(ats.getServerATS(ATS.Server.VANILLA_ANARCHY).getLastActivity()), true)
