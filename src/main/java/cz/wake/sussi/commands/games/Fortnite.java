    ...
    public void onCommand(User sender, MessageChannel channel, Message message, String[] args, Member member, EventWaiter w) {
        try {
            if (!member.getRoles().contains(member.getGuild().getRoleById("430730941728817154"))) {
                ...
                MessageUtils.sendAutoDeletedMessage(member.getAsMention() + " nastavil/a sis roli `Fortnite`!", 5000L, channel);
            } else {
                member.getGuild().removeRoleFromMember(member, member.getGuild().getRoleById("430730941728817154")).queue();
                message.delete().queue();
                MessageUtils.sendAutoDeletedMessage(member.getAsMention() + " odebral/a sis roli `Fortnite`!", 5000L, channel);
            }
        } catch (Exception ex){
            ex.printStackTrace();
        }
