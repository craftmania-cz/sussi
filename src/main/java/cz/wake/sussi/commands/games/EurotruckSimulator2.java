    ...
    public void onCommand(User sender, MessageChannel channel, Message message, String[] args, Member member, EventWaiter w) {
        try {
            if (!member.getRoles().contains(member.getGuild().getRoleById("602251414705668137"))) {
                member.getGuild().addRoleToMember(member, member.getGuild().getRoleById("602251414705668137")).queue();
                message.delete().queue();
                MessageUtils.sendAutoDeletedMessage(member.getAsMention() + " nastavil/a jsi si roli `Euro Truck Simulator 2`!", 5000L, channel);
            } else {
                member.getGuild().removeRoleFromMember(member, member.getGuild().getRoleById("602251414705668137")).queue();
                message.delete().queue();
